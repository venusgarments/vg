...
export const addItemToCart = (reqData) => async (dispatch) => {
  try {
    dispatch({ type: ADD_ITEM_TO_CART_REQUEST });
    console.log("axios cart data", reqData)
    ...
    return data; // ✅ return result to use in `.then()` on frontend
  } catch (error) {
    dispatch({
      type: ADD_ITEM_TO_CART_FAILURE,
      payload:
    ...
    throw error; // ✅ important so error can be caught in UI
  }
};
...
export const removeCartItem = (reqData) => async (dispatch) => {
  try {
    ...
    // ✅ Fetch latest cart data after item removal
...
export const updateCartItem = (reqData) => async (dispatch) => {
  try {
    ...
    // ✅ Fetch latest cart data after item update
    dispatch(getCart(reqData.jwt));
  } catch (error) {
    dispatch({
