...
const getUserProfileByToken = async (token) => {
  try {
    ...
    console.log("Profile Sync - Decoded userId:", userId);

    const user = await User.findById(userId).populate("addresses");

    if (!user) {
      console.error(
        `❌ getUserProfileByToken - User does not exist with id: ${userId}`
      );
    ...
    console.error("❌ getUserProfileByToken Error:", error.message);
    throw new Error(error.message);
  }
};
  ...
  try {
    ...
    if (existingOtp && existingOtp.createdAt) {
      ...
      if (existingOtp.attempts >= 3 && minutesSinceLast < 1) {
        existingOtp.blockedUntil = new Date(now.getTime() + 1 * 60 * 1000); // 1 min block for testing
        await existingOtp.save();
        throw new Error("Too many OTP requests. Try again soon.");
      }
    }
    ...
    console.error("❌ verifyEmailService Error:", error.message);
    throw new Error(error.message);
  }
};
...
const confirmOtpService = async (email, userOtp) => {
  try {
    ...
    console.error("❌ confirmOtpService Error:", error.message);
    throw new Error(error.message);
  }
};
...
const sendResetOtpService = async (email) => {
  try {
    ...
    console.error("❌ sendResetOtpService Error:", error.message);
    throw new Error(error.message);
  }
};
